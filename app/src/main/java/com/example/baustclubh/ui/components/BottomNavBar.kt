...
fun BottomNavBar(navController: NavController) {
    ...
    NavigationBar(
        ...
        // 🎓 ৩. Alumni (লাইব্রেরির জায়গায় নতুন যুক্ত করা হলো)
        NavigationBarItem(
            icon = { Icon(Icons.Outlined.School, contentDescription = null, modifier = Modifier.size(24.dp)) }, // গ্র্যাজুয়েশন ক্যাপ আইকন
            label = { Text("Alumni", fontSize = 10.sp) },
            selected = currentRoute == "alumni",
            onClick = {
                if (currentRoute != "alumni") {
                    navController.navigate("alumni") { launchSingleTop = true }
                }
            },
            colors = navigationBarItemColors()
        )

        // ৪. Events (স্মুথ নেভিগেশন ফিক্স করা হয়েছে)
        NavigationBarItem(
            icon = { Icon(Icons.Outlined.CalendarMonth, contentDescription = null, modifier = Modifier.size(24.dp)) },
            label = { Text("Events", fontSize = 10.sp) },
            selected = currentRoute == "event_list",
